...
class SiteController extends Controller
{
    ...
    public function actionIndex($category = '')
    {
        ...
        $products = Product::find()
            ->select(['product.*'])
            ->joinWith('category')
            ->where(['LOWER(product_category.name)' => 'new arrivals'])
            ->all();
        $populars = [];
        $populars = Product::find()
            ->select(['product.*'])
            ->joinWith('category')
            ->where(['LOWER(product_category.name)' => 'popular items'])
            ->all();
        ...
        if (empty($products or $populars) && $category == '') {
            $products = Product::find()->all();
            $populars = Product::find()->all();
        }
    ...
    {
        return $this->render('blog_details');
    }
